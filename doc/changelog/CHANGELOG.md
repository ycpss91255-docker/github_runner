...
- Runner labels are now configurable instead of hard-coded to `gpu`.
  ...
  `script/configure.sh`.
- CI (`shellcheck` / `bats` / `coverage`) now runs on the self-hosted runner
  (`runs-on: [self-hosted, Linux, X64]`) instead of GitHub-hosted
  `ubuntu-latest` — the cached test-tools image and no VM spin-up make repeat
  runs faster. Fork-PR safety is unchanged (org outside-collaborator approval
  gate, see Security model).
