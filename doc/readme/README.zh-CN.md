...
| `script/remove-runner.sh` | 取消注册 + uninstall systemd service + 删目录。预设 prompt 确认，`--yes` 跳过，`--dry-run` 预览 |
...
覆盖率在 `kcov/kcov` 内跑（Debian，内含 `kcov`，但没有 `bats`）；`bats-core`
由 `script/fetch-bats.sh` 以固定版本 + sha256 验证后缓存在 host，再挂进
container。运行期不安装任何软件包，且 coverage container 以 `--network none`
启动，所以跑过一次 `just pull` 之后，整条检查完全不需要对外网络。本机跟 CI
共用相同 image。
...
```bash
just pull       # 拉 test-tools + kcov image 并缓存 bats（首次）
just lint       # shellcheck + hadolint + repo 结构 lint
                # （ADR 格式、changelog 条目、4 语言 README、文档引用）
just test       # 整套 bats：unit + integration（在 docker 内）
just test-unit  # 单一函数／单一文件
just test-integration  # 多个 script 或 library 一起
...
1. **外部贡献者 approval gate**（org Settings → Actions → General →
   「Require approval for all outside collaborators」）。挡住 fork PR 在
   runner 上任意执行 code，直到 maintainer 按「Approve and run」。
...
移除**单一** runner(deregister + 卸载 systemd service + 删目录)。默认会
prompt,先预览再确认:

```bash
./script/remove-runner.sh --dry-run org <your-org>   # 显示会移除什么
./script/remove-runner.sh org <your-org>             # org runner(会 prompt)
./script/remove-runner.sh repo <owner> <repo>        # repo runner(会 prompt)
./script/remove-runner.sh --yes org <your-org>       # 跳过 prompt(非 TTY 必填)
```
