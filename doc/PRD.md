...
| Changelog entry present | The changed paths of a branch | `script/lint-changelog.sh` | The `lint-changelog` recipe in the `justfile` (a prerequisite of `lint` and `lint-host`); the CI `changelog-lint` job | A change an operator can observe must say so in `doc/changelog/CHANGELOG.md` (§0.7). Documentation, tests and repo metadata never warrant an entry — a gate that fires on a typo fix is one people route around |
| Four-language README structure | `README.md` and the three translations under `doc/readme/` | `script/lint-readme-sync.sh` | The `lint-readme-sync` recipe (same prerequisite); the CI `readme-sync-lint` job | A partial update leaves the other readers told less. Heading levels and fenced-block counts are compared, not heading text — the headings are translated |
| No `file:line` citations, no hardcoded counts | The project's own markdown documentation | `script/lint-doc-citations.sh` | The `lint-doc-citations` recipe (same prerequisite); the CI `doc-citation-lint` job | Both are copies of what the tree already states (invariant 3), and both go stale with no signal. Fenced code blocks, URLs, and lines carrying the `<!-- doc-lint-allow -->` marker are not checked |
| Merge gate | Every job above: shellcheck / adr-lint / bats / go / coverage / changelog-lint / readme-sync-lint / doc-citation-lint | `ci-rollup` | The `ci-rollup` job in `ci.yaml` | Branch protection only needs to track **one** stable check name, leaving sub-jobs free to be renamed or split |
